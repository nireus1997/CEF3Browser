    ...
    case WM_PAINT:
        {
            ...
            if( m_bAlphaBackground)
            {
                ...
                PAINTSTRUCT ps = { 0 };
               HDC hdcPaint = ::BeginPaint(m_hWndPaint, &ps);
                ...
                {
                    RECT rcClipClient = {0};
                    memcpy(&rcClipClient, &rcClient, sizeof(RECT));
                    DrawWindowShadow(m_hWndPaint,rcClipClient);
                    m_pRoot->DoPaint(m_hDcBackground, rcClipClient);
                ...
                ::UpdateLayeredWindow(m_hWndPaint,hdcPaint,&pt,&szWindow,m_hDcBackground,&ptSrc,0,&blendPixelFunction,ULW_ALPHA);
                ::EndPaint(m_hWndPaint, &ps);
...
bool CPaintManagerUI::IsAlphaBackground() const
{
    return m_bAlphaBackground;
}

void CPaintManagerUI::SetIsWindowShadow(bool bShadow)
{
    m_bIsShadow = bShadow;
}

bool CPaintManagerUI::IsWindowShadow() const
{
    return m_bIsShadow;
}

void CPaintManagerUI::SetHasWindowShadow(bool bShadow)
{
    m_bHasShadow = bShadow;
}

bool CPaintManagerUI::HasWindowShadow() const
{
    return m_bHasShadow;
}

void CPaintManagerUI::SetWindowShadowImage( LPCTSTR pStrImage )
{
    m_sShadowImage = pStrImage;
}

void CPaintManagerUI::SetTopleftShadowRect( int x, int y, int w, int h )
{
    m_shadowInfo.rTopLeft.left = x;
    m_shadowInfo.rTopLeft.top = y;
    m_shadowInfo.rTopLeft.right = m_shadowInfo.rTopLeft.left + w;
    m_shadowInfo.rTopLeft.bottom = m_shadowInfo.rTopLeft.top + h;
}

void CPaintManagerUI::SetToprightShadowRect( int x, int y, int w, int h )
{
    m_shadowInfo.rTopRight.left = x;
    m_shadowInfo.rTopRight.top = y;
    m_shadowInfo.rTopRight.right = m_shadowInfo.rTopRight.left + w;
    m_shadowInfo.rTopRight.bottom = m_shadowInfo.rTopRight.top + h;
}

void CPaintManagerUI::SetBottomleftShadowRect( int x, int y, int w, int h )
{
    m_shadowInfo.rBottomLeft.left = x;
    m_shadowInfo.rBottomLeft.top = y;
    m_shadowInfo.rBottomLeft.right = m_shadowInfo.rBottomLeft.left + w;
    m_shadowInfo.rBottomLeft.bottom = m_shadowInfo.rBottomLeft.top + h;
}

void CPaintManagerUI::SetBottomrightShadowRect( int x, int y, int w, int h )
{
    m_shadowInfo.rBottomRight.left = x;
    m_shadowInfo.rBottomRight.top = y;
    m_shadowInfo.rBottomRight.right = m_shadowInfo.rBottomRight.left + w;
    m_shadowInfo.rBottomRight.bottom = m_shadowInfo.rBottomRight.top + h;
}

void CPaintManagerUI::CalMiddleShadowRect()
{
    m_shadowInfo.rTopMiddle.left = m_shadowInfo.rTopLeft.right;
    m_shadowInfo.rTopMiddle.right = m_shadowInfo.rTopRight.left;
    m_shadowInfo.rTopMiddle.top = 0;
    m_shadowInfo.rTopMiddle.bottom = m_shadowInfo.rTopLeft.bottom;

    m_shadowInfo.rBottomMiddle.left = m_shadowInfo.rBottomLeft.right;
    m_shadowInfo.rBottomMiddle.right = m_shadowInfo.rBottomRight.left;
    m_shadowInfo.rBottomMiddle.top = m_shadowInfo.rBottomLeft.top;
    m_shadowInfo.rBottomMiddle.bottom = m_shadowInfo.rBottomLeft.bottom;

    m_shadowInfo.rLeftMiddle.left = m_shadowInfo.rTopLeft.left;
    m_shadowInfo.rLeftMiddle.right = m_shadowInfo.rTopLeft.right;
    m_shadowInfo.rLeftMiddle.top = m_shadowInfo.rTopLeft.bottom;
    m_shadowInfo.rLeftMiddle.bottom = m_shadowInfo.rBottomLeft.top;

    m_shadowInfo.rRightMiddle.left = m_shadowInfo.rTopRight.left;
    m_shadowInfo.rRightMiddle.right = m_shadowInfo.rTopRight.right;
    m_shadowInfo.rRightMiddle.top = m_shadowInfo.rTopRight.bottom;
    m_shadowInfo.rRightMiddle.bottom = m_shadowInfo.rBottomRight.top;
}

void CPaintManagerUI::DrawWindowShadow(HWND wnd,RECT& rcPaint)
{
     //m_hDcBackground = GetDC(wnd);
    TWindowShadowInfo realShadowInfo;
    CalRealShadowRect(rcPaint, realShadowInfo);
    //RECT rcWindow = {0, 0, m_szInitWindowSize.cx, m_szInitWindowSize.cy};

    TCHAR sRenderString[512] = {0};

    GenerateShadowRenderString(m_shadowInfo.rTopLeft, realShadowInfo.rTopLeft, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rTopMiddle, realShadowInfo.rTopMiddle, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rTopRight, realShadowInfo.rTopRight, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rLeftMiddle, realShadowInfo.rLeftMiddle, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rRightMiddle, realShadowInfo.rRightMiddle, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rBottomLeft, realShadowInfo.rBottomLeft, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rBottomMiddle, realShadowInfo.rBottomMiddle, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    GenerateShadowRenderString(m_shadowInfo.rBottomRight, realShadowInfo.rBottomRight, sRenderString);
    CRenderEngine::DrawImageString(m_hDcBackground, this, rcPaint, rcPaint, sRenderString);

    rcPaint.left = realShadowInfo.rTopLeft.right;
    rcPaint.top = realShadowInfo.rTopLeft.bottom;
    rcPaint.right = realShadowInfo.rTopRight.left;
    rcPaint.bottom = realShadowInfo.rLeftMiddle.bottom;
}

void CPaintManagerUI::CalRealShadowRect( RECT& rcClient, TWindowShadowInfo& realShadowInfo )
{
	realShadowInfo.rTopLeft.left = 0;
	realShadowInfo.rTopLeft.right = m_shadowInfo.rTopLeft.right;
	realShadowInfo.rTopLeft.top = 0;
	realShadowInfo.rTopLeft.bottom = m_shadowInfo.rTopLeft.bottom;

	realShadowInfo.rTopMiddle.left = realShadowInfo.rTopLeft.right;
	realShadowInfo.rTopMiddle.right = rcClient.right-(m_shadowInfo.rTopRight.right-m_shadowInfo.rTopRight.left);
	realShadowInfo.rTopMiddle.top = 0;
	realShadowInfo.rTopMiddle.bottom = m_shadowInfo.rTopLeft.bottom;

	realShadowInfo.rTopRight.left = realShadowInfo.rTopMiddle.right;
	realShadowInfo.rTopRight.right = rcClient.right;
	realShadowInfo.rTopRight.top = 0;
	realShadowInfo.rTopRight.bottom = m_shadowInfo.rTopRight.bottom;

	realShadowInfo.rLeftMiddle.left = 0;
	realShadowInfo.rLeftMiddle.right = m_shadowInfo.rTopLeft.right;
	realShadowInfo.rLeftMiddle.top = m_shadowInfo.rTopLeft.bottom;
	realShadowInfo.rLeftMiddle.bottom = rcClient.bottom - (m_shadowInfo.rBottomLeft.bottom-m_shadowInfo.rBottomLeft.top);

	realShadowInfo.rRightMiddle.left = realShadowInfo.rTopRight.left;
	realShadowInfo.rRightMiddle.right = rcClient.right;
	realShadowInfo.rRightMiddle.top = realShadowInfo.rTopRight.bottom;
	realShadowInfo.rRightMiddle.bottom = realShadowInfo.rLeftMiddle.bottom;

	realShadowInfo.rBottomLeft.left = 0;
	realShadowInfo.rBottomLeft.right = m_shadowInfo.rBottomLeft.right;
	realShadowInfo.rBottomLeft.top = realShadowInfo.rLeftMiddle.bottom;
	realShadowInfo.rBottomLeft.bottom = rcClient.bottom;

	realShadowInfo.rBottomMiddle.left = realShadowInfo.rBottomLeft.right;
	realShadowInfo.rBottomMiddle.right = realShadowInfo.rTopMiddle.right;
	realShadowInfo.rBottomMiddle.top = realShadowInfo.rBottomLeft.top;
	realShadowInfo.rBottomMiddle.bottom = rcClient.bottom;

	realShadowInfo.rBottomRight.left = realShadowInfo.rBottomMiddle.right;
	realShadowInfo.rBottomRight.right = rcClient.right;
	realShadowInfo.rBottomRight.top = realShadowInfo.rBottomLeft.top;
	realShadowInfo.rBottomRight.bottom = rcClient.bottom;

}

void CPaintManagerUI::GenerateShadowRenderString( RECT& src, RECT& dst, LPTSTR sRenderString )
{
    _stprintf(sRenderString, _T("file='%s' dest='%u,%u,%u,%u' source='%u,%u,%u,%u'"), (LPCTSTR)m_sShadowImage,
