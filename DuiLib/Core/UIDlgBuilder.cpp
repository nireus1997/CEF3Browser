...
CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    if( pManager ) {
        ...
        if( _tcscmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    else if( _tcscmp(pstrName, _T("hasshadow")) == 0 ) {
                        pManager->SetHasWindowShadow(_tcscmp(pstrValue, _T("true")) == 0 );
                    }
                    else if( _tcscmp(pstrName, _T("shadow")) == 0 ) {
                        pManager->SetIsWindowShadow(_tcscmp(pstrValue, _T("true")) == 0 );
                    }
                    else if( _tcscmp(pstrName, _T("shadowimage")) == 0 ) {
                        pManager->SetWindowShadowImage(pstrValue);
                    }
                    else if( _tcscmp(pstrName, _T("shadowtopleft")) == 0 ) {
                        LPTSTR pstr = NULL;
                        int x = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
                        int y = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        int w = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
                        int h = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        pManager->SetTopleftShadowRect(x, y, w, h);
                    }
                    else if( _tcscmp(pstrName, _T("shadowtopright")) == 0 ) {
                        LPTSTR pstr = NULL;
                        int x = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
                        int y = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        int w = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
                        int h = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        pManager->SetToprightShadowRect(x, y, w, h);
                    }
                    else if( _tcscmp(pstrName, _T("shadowbottomleft")) == 0 ) {
                        LPTSTR pstr = NULL;
                        int x = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
                        int y = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        int w = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
                        int h = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        pManager->SetBottomleftShadowRect(x, y, w, h);
                    }
                    else if( _tcscmp(pstrName, _T("shadowbottomright")) == 0 ) {
                        LPTSTR pstr = NULL;
                        int x = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
                        int y = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        int w = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
                        int h = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr); 
                        pManager->SetBottomrightShadowRect(x, y, w, h);
                    }
        ...
        if ( pManager->IsWindowShadow() )
        {
            pManager->CalMiddleShadowRect();
        }
    }
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 14:
				...
				else if (_tcscmp(pstrClass, DUI_CTR_CIRCINALBUTTON) == 0)     pControl = new CCircinalButtonUI;
                break;
            case 15:
