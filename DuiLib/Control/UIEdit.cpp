...
namespace DuiLib
{
	...
	void CEditUI::SetWaterText(LPCTSTR pstrText)
	{
		m_sWaterText = pstrText;
		Invalidate();
	}
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("watertext")) == 0 ) SetWaterText(pstrValue);
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		{
			if (m_uButtonState == 0 
				|| (m_uButtonState & UISTATE_FOCUSED) != 0 
				|| (m_uButtonState & UISTATE_HOT) != 0)
			{
				sText = m_sWaterText;
			}
			else
